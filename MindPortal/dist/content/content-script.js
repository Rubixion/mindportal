import { e as extractDomain, d as domainMatchesList } from "../chunks/utils-DXHU2JcO.js";
...
async function main() {
  ...
  const autoBlock = settings.pomodoroAutoFocusMode && session.pomodoroActive && !session.pomodoroIsBreak;
...
function showOverlay(domain, mode, countdownSeconds, intention, storage) {
  ...
  if (mode === "block") {
    const intentionNote = intention ? `<p style="font-size:13px;color:rgba(105,130,216,0.8);margin-bottom:20px;line-height:1.4">
    ...
    const countdownId2 = "mp-countdown-num";
    const continueId2 = "mp-continue-btn";
    card.innerHTML = `
      ${ollieSvg}
      <h2 style="color:rgba(255,255,255,0.92);font-size:20px;font-weight:700;margin-bottom:10px;letter-spacing:-0.02em">
      ...
      <div style="display:flex;gap:10px;justify-content:center;margin-bottom:18px">
        <button id="mp-go-back" style="${btnPrimary}">← Go back</button>
        <button id="${continueId2}" style="${btnGhost}" ${mode === "countdown" ? "disabled" : ""}>
          ${mode === "countdown" ? `Continue in <span id="${countdownId2}">${countdownSeconds}</span>s` : "Continue anyway"}
        </button>
      </div>
  ...
  function handleContinue() {
    const cb = document.getElementById("mp-dismiss-today");
    if (cb?.checked) {
